...
export default function TransitionsModal(props) {
  ...
  return (
    ...
    // Jinhoo change
    <div>
      <Button className={classes.modalBtn} size="small" onClick={handleOpen}>Details</Button>
      <Modal
        aria-labelledby="transition-modal-title"
        aria-describedby="transition-modal-description"
        className={classes.modal}
      ...
      >
        ...
        <>
          ...
          <div className={classes.spanDivs}>
          <span className={classes.span}>Event: </span>
          <span className={classes.title3}>{props.selectedEvent.event_name}</span>
          </div>
          <div className={classes.spanDivs}>
          <span className={classes.span}>Host: </span>
          <span className={classes.font}>{props.selectedEvent.host_email}</span>
          </div>

          <div className={classes.spanDivs}>
            <span className={classes.span}>Date: </span>
            <span className={classes.font}>{props.selectedEvent.date}</span>
          </div>

          <div className={classes.spanDivs}>
            <span className={classes.span}>Time: </span>
            <span className={classes.font}>{props.selectedEvent.time}</span>
          </div>

          <div className={classes.spanDivs}>
            <span className={classes.span}>Location: </span>
            <span className={classes.font}>{props.selectedEvent.location}</span>
          </div>

          <div className={classes.spanDivs}>
            <span className={classes.span}>Description: </span>
            <span className={classes.font}>{props.selectedEvent.description}</span>
          </div>
          </div>
         </>
